...
class FID:
	...
	def __init__(self, fid):
		...
		# Make a function dictionary to simplify code later
		# Entry format: [ptr_function, n_parameters]
		self.function = {}
		self.function['lz'] = [lorentzian, 4]
		self.function['sl'] = [soft_lorentzian, 5]
		self.function['ex'] = [exponential, 4]
		self.function['p1'] = [pol1, 2]
		self.function['p2'] = [pol2, 3]
		self.function['p3'] = [pol3, 4]
		self.function['os'] = [osc, 4]
		self.function['an'] = [analytical_peak, 5]

		# # I'm doubling entries, but its fine for now.
		# for key, elem in self.function.iteritems():
		# 	self.function[self.method[key]] = self.function[key]
...
#		self.fitFunction(id)
	...
	def getAnalyticalFreq(self):
		...
		self.fitAnalyticalPeak()

		return self.par[0] # Frequency

	def fitAnalyticalPeak(self):
	...
	def getLorentzianFreq(self):
		...
		formula = '[2] / (1 + abs(x - [0])^2 / (0.5 * [1])^2  + [3] * x'
		f_lz = TF1('f_lz', formula, 0, 100); # Limits should be okay
		...
		for i, p in enumerate(self.guess[:4]):
			f_lz.SetParameter(i, p)

		h_fid = TH1F("h_fid", x.shape[0], x[0], x[-1])
		for i, p in enumerate(y):
			h_fid.SetBinContent(i, p)
	...
	def getExponentialFreq(self):
		...
		par, cov = curve_fit(exp_peak, x, y, p0=g, maxfev=2000)
		self.guess[:par.shape[0]] = par.tolist()
	...
	def getExponentialFit(self):
		...
		fit = exp_peak(x, par[0], par[1], par[2], par[3])
...
def analytical_peak(x, x0, gamma, a, b, phi):
	...
	num = a * (x0**2 - 0.5 * gamma * w0 * np.sin(2 * phi))
